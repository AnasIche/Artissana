...
const Loader = () => {
  useEffect(() => {
    // Animation for the SVG path (drawing effect)
    gsap.fromTo(
      "#svg path", 
    ...
    // Fade out the loading page and reveal the logo name
    gsap.fromTo(
      ".loading-page",
      { opacity: 1 },
    ...
    // Animation for the logo name (slide and fade-in)
    gsap.fromTo(
      ".logo-name",
      { y: 50, opacity: 0 },
  ...
  return (
    <div className="loading-page">
      ...
      <div className="name-container">
        <div className="logo-name">Artisal</div>
      </div>
    </div>
  );
};
